...
class ProductsController extends BaseController {
	...
	public function index(){
		...
		foreach($res as $key=>$val){
			$res_data = M('products_data')->where(array('pid'=>$val['id']))->select();
	...
	public function edit() {
		...
		if (IS_POST) {
			...
			if ($data) {
				...
				if($id == 0){
					...
					foreach($data as $key=>$val){
						...
						$val['time'] = time();
						$result = M('products_data')->add($val);
					}
					if($res && $result){
					...
					foreach($data as $key=>$val){
						...
						$val['custom'] = serialize($val['custom']);
						$val['time'] = time();
						$result = M('products_data')->where($where)->save($val);
					}
					if($res && $result){
						$this->ajax_json('修改操作成功',1,$data);
					}else{
			...
			if ($id) {
				...
				$langdata = M('products_data')->where(array('pid'=>$id))->select();
