...
<body>
	<nav class="breadcrumb"><i class="Hui-iconfont">&#xe67f;</i> 首页 <span class="c-gray en">&gt;</span> 文章管理 <span class="c-gray en">&gt;</span> 栏目列表
	...
	<div class="pd-20">
		<div class="cl pd-5 bg-1 bk-gray">
			<span class="l">
					...
					<a class="btn btn-primary radius" onclick="article_add('添加栏目','<?php echo U('add');?>')" href="javascript:;">
						<i class="Hui-iconfont">&#xe600;</i> 添加栏目
					</a>
					<a class="btn btn-primary radius" onclick="reflash_cache('<?php echo U('reflash');?>','nav')" href="javascript:;">
			...
			<table class="table table-border table-bordered table-bg table-hover table-sort">
				...
				<tbody>
					<?php if(is_array($data["data"])): $i = 0; $__LIST__ = $data["data"];if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?><tr class="text-c">
							...
							<td class="text-l"><?php echo ($vo["title_show"]); echo ($vo["name"]); ?></td>
							<td class="text-l"><?php echo (get_span($vo["url"],$vo['id'],'url')); ?></td>
							<td class="text-l"><?php echo (get_span($vo["template"],$vo['id'],'template')); ?></td>
							<td><?php echo (getmodularname($vo["modular_id"])); ?></td>
