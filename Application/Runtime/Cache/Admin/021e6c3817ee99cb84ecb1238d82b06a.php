		...
		<form method="post" class="form form-horizontal" id="form-setting-edit">
			<div class="row cl">
				...
				<div class="formControls col-3">
					<input type="text" class="input-text" value="<?php echo ($data["name"]); ?>" placeholder="配置函数名称" id="" name="name">
				</div>
				<label class="form-label col-2"><span class="c-red">*</span>标题：</label>
				<div class="formControls col-3">
					<input type="text" class="input-text" value="<?php echo ($data["title"]); ?>" placeholder="显示标题" id="" name="title">
				</div>
			</div>
			<div class="row cl">
				...
				<div class="formControls col-2">
					<span class="select-box">
						<select name="groups" class="select">
							...
							<?php if(is_array($group_list)): $i = 0; $__LIST__ = $group_list;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?><option value="<?php echo ($i); ?>" <?php echo $data['groups']==$i?'selected=""':'';?>><?php echo ($vo); ?></option><?php endforeach; endif; else: echo "" ;endif; ?>
						</select>
					</span>
				</div>
				...
				<div class="formControls col-2">
					<span class="select-box">
						<select name="type" class="select">
							...
							<?php if(is_array($type_list)): $i = 0; $__LIST__ = $type_list;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?><option value="<?php echo ($i); ?>" <?php echo $data['type']==$i?'selected=""':'';?>><?php echo ($vo); ?></option><?php endforeach; endif; else: echo "" ;endif; ?>
						</select>
					</span>
				</div>
			</div>
			...
			<div class="row cl">
				...
				<div class="formControls col-10">
					<textarea name="item" cols="" rows="" class="textarea" placeholder="配置项，例：1:开启,2:关闭 （只用于下拉框）"><?php echo ($data["item"]); ?></textarea>
				</div>
			</div>
			<div class="row cl">
				<label class="form-label col-2">配置值：</label>
				<div class="formControls col-10">
					<textarea name="value" cols="" rows="" class="textarea" placeholder="配置值"><?php echo ($data["value"]); ?></textarea>
				</div>
			</div>
			<div class="row cl">
				<label class="form-label col-2">备注：</label>
				<div class="formControls col-10">
					<input type="text" name="note" class="input-text" placeholder="备注" value="<?php echo ($data["note"]); ?>"/>
				</div>
			</div>
			<div class="row cl">
