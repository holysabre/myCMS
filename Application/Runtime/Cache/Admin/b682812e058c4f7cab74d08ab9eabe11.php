		...
		<div class="pd-20">
			<div class="cl pd-5 bg-1 bk-gray"> 
				<span class="l">
					...
					<a class="btn btn-primary radius" onclick="article_add('添加管理员','<?php echo U('add');?>')" href="javascript:;">
				...
				<table class="table table-border table-bordered table-bg table-hover table-sort">
					...
					<tbody>
						<?php if(is_array($data["data"])): $i = 0; $__LIST__ = $data["data"];if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?><tr class="text-c">
								...
								<td class="text-l"><?php echo (get_span($vo["username"],$vo['id'],'username')); ?></td>
								<td class="text-m"><?php echo getAuthName($vo['roleid']);?></td>
								<td><?php echo (get_strtotime($vo["regtime"],full)); ?></td>
								<td><?php echo (get_strtotime($vo["lasttime"],full)); ?></td>
