...
<script type="text/javascript" src="/myCMS/Application/Admin/View/Static/lib/kindeditor/kindeditor-min.js"></script>
		<title><?php echo ($top_title); ?>列表</title>
	</head>

	<body>
		<nav class="breadcrumb"><i class="Hui-iconfont">&#xe67f;</i> 首页 <span class="c-gray en">&gt;</span> <?php echo ($top_title); ?>管理 <span class="c-gray en">&gt;</span> <?php echo ($top_title); ?>列表 
		...
		<div class="pd-20">
			<div class="cl pd-5 bg-1 bk-gray"> 
				<span class="l">
					...
					<a class="btn btn-primary radius" onclick="article_add('添加菜单','<?php echo U('add');?>')" href="javascript:;">
						<i class="Hui-iconfont">&#xe600;</i> 添加菜单
					</a>
					<a class="btn btn-primary radius" onclick="reflash_cache('<?php echo U('reflash');?>','language')" href="javascript:;">
				...
				<table class="table table-border table-bordered table-bg table-hover table-sort">
					...
					<tbody>
						<?php if(is_array($data["data"])): $i = 0; $__LIST__ = $data["data"];if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?><tr class="text-c">
								...
								<td class="text-l"><?php echo ($vo["title_show"]); echo (get_span($vo["name"],$vo['id'],'name')); ?></td>
								<td class="text-l"><?php echo (get_span($vo["mark"],$vo['id'],'mark')); ?></td>
								<td><?php echo (get_toggle($vo["status"],$vo['id'],'status')); ?></td>
								<td><?php echo (get_span($vo["sort"],$vo['id'],'sort')); ?></td>
								<td><?php echo $vo['is_system'] == 1 ? '<i class="Hui-iconfont" style="color:red;">&#xe6a7;</i>' : '<i class="Hui-iconfont">&#xe6a6;</i>';?></td>
								<td class="f-14 td-manage">
									<a style="text-decoration:none" class="ml-5" href="<?php echo U('edit_lang?mark='.$vo['mark']);?>" title="编辑语言">
